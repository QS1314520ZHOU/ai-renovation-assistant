...
CONTRACT_PARSE_PROMPT = (
    '你是一个专业的中国室内装修合同审核律师。请将以下装修合同内容进行分析，并提取核心条款风险，输出为结构化JSON格式。\n\n'
    '要求提取和分析以下内容：\n'
    '1. 付款比例 (payment_terms): 提取各阶段的付款比例（如开工、水电、泥瓦、竣工）。行业常规健康比例为开工30%-水电30%-泥瓦30%-竣工10%。如果首付过高(>40%)或中期过早结清，需要标记风险。\n'
    '2. 增项约定 (add_on_terms): 合同中是否有增项不超过合同总价5%或类似限制。如果没有，属于中/高风险。\n'
    ...
    '请严格按照JSON格式输出，示例：\n'
    '{"score":85,"payment_terms":{"details":"开工40%，水电40%，竣工20%","is_healthy":false,"analysis":"说明"},'
    '"risks":[{"category":"付款比例","risk_level":"medium","original_text":"原文","risk_point":"风险点","suggestion":"建议"}],'
    '"recommendations":["建议1"],"summary":"总结"}\n\n'
    '以下是合同内容：\n'
)
...
class ContractService:
    ...
    async def upload_and_parse(self, project_id: uuid.UUID, file):
        ...
        try:
            ...
            json_data = self._extract_json(parse_result.choices[0].message.content)
            if not json_data:
